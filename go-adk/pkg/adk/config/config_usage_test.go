...
func TestValidateAgentConfigUsage_ValidMinimal(t *testing.T) {
	config := &types.AgentConfig{
		Model:       &types.OpenAI{BaseModel: types.BaseModel{Type: types.ModelTypeOpenAI, Model: "gpt-4"}},
...
func TestValidateAgentConfigUsage_HttpToolMissingURL(t *testing.T) {
	config := &types.AgentConfig{
		Model:       &types.OpenAI{BaseModel: types.BaseModel{Type: types.ModelTypeOpenAI, Model: "gpt-4"}},
		Instruction: "test",
		HttpTools: []types.HttpMcpServerConfig{
			{Params: types.StreamableHTTPConnectionParams{Url: ""}},
		},
	}
	err := ValidateAgentConfigUsage(config)
	if err == nil {
...
func TestValidateAgentConfigUsage_SseToolMissingURL(t *testing.T) {
	config := &types.AgentConfig{
		Model:       &types.OpenAI{BaseModel: types.BaseModel{Type: types.ModelTypeOpenAI, Model: "gpt-4"}},
		Instruction: "test",
		SseTools: []types.SseMcpServerConfig{
			{Params: types.SseConnectionParams{Url: ""}},
		},
	}
	err := ValidateAgentConfigUsage(config)
	if err == nil {
...
func TestValidateAgentConfigUsage_RemoteAgentMissingURL(t *testing.T) {
	config := &types.AgentConfig{
		Model:       &types.OpenAI{BaseModel: types.BaseModel{Type: types.ModelTypeOpenAI, Model: "gpt-4"}},
		Instruction: "test",
		RemoteAgents: []types.RemoteAgentConfig{
			{Name: "agent1", Url: ""},
		},
	}
	err := ValidateAgentConfigUsage(config)
