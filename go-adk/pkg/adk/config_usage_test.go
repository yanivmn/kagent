...
func TestValidateAgentConfigUsage_ValidMinimal(t *testing.T) {
	config := &core.AgentConfig{
		Model:       &core.OpenAI{BaseModel: core.BaseModel{Type: core.ModelTypeOpenAI, Model: "gpt-4"}},
...
func TestValidateAgentConfigUsage_HttpToolMissingURL(t *testing.T) {
	config := &core.AgentConfig{
		Model:       &core.OpenAI{BaseModel: core.BaseModel{Type: core.ModelTypeOpenAI, Model: "gpt-4"}},
		Instruction: "test",
		HttpTools: []core.HttpMcpServerConfig{
			{Params: core.StreamableHTTPConnectionParams{Url: ""}},
		},
	}
	err := ValidateAgentConfigUsage(config)
	if err == nil {
...
func TestValidateAgentConfigUsage_SseToolMissingURL(t *testing.T) {
	config := &core.AgentConfig{
		Model:       &core.OpenAI{BaseModel: core.BaseModel{Type: core.ModelTypeOpenAI, Model: "gpt-4"}},
		Instruction: "test",
		SseTools: []core.SseMcpServerConfig{
			{Params: core.SseConnectionParams{Url: ""}},
		},
	}
	err := ValidateAgentConfigUsage(config)
	if err == nil {
...
func TestValidateAgentConfigUsage_RemoteAgentMissingURL(t *testing.T) {
	config := &core.AgentConfig{
		Model:       &core.OpenAI{BaseModel: core.BaseModel{Type: core.ModelTypeOpenAI, Model: "gpt-4"}},
		Instruction: "test",
		RemoteAgents: []core.RemoteAgentConfig{
			{Name: "agent1", Url: ""},
		},
	}
	err := ValidateAgentConfigUsage(config)
