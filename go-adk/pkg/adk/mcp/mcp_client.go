...
func (r *MCPToolRegistry) fetchToolsFromServer(
	...
	initTimeout := a2a.MCPInitTimeout
	if timeout != nil && *timeout > 0 {
		...
		if initTimeout > a2a.MCPInitTimeoutMax {
			initTimeout = a2a.MCPInitTimeoutMax
		}
	}
		...
		if r.logger.GetSink() != nil {
			...
			if len(inputSchema) > 0 {
				if schemaBytes, err := json.Marshal(inputSchema); err == nil {
					...
					if len(schemaJSON) > a2a.SchemaJSONMaxLength {
						schemaJSON = schemaJSON[:a2a.SchemaJSONMaxLength] + "... (truncated)"
					}
				}
			}
...
func (r *MCPToolRegistry) FetchToolsFromHttpServer(ctx context.Context, config types.HttpMcpServerConfig) error {
...
func (r *MCPToolRegistry) FetchToolsFromSseServer(ctx context.Context, config types.SseMcpServerConfig) error {
	url := config.Params.Url
	headers := config.Params.Headers
	if headers == nil {
		headers = make(map[string]string)
